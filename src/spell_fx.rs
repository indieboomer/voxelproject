...
#[derive(Default)]
pub struct Effects {
    bursts: Vec<(Vec3, Vec3, f32)>,
}
impl Effects {
    pub fn cast(&mut self, origin: Vec3, target: Vec3) {
        if !origin.is_finite()
            || !target.is_finite()
            || origin.abs().max_element() > 1_000_000.
            || target.abs().max_element() > 1_000_000.
            || origin.distance(target) > 24.
        {
            return;
        }
    ...
    pub fn update(&mut self, dt: f32) {
        ...
        for (_, _, age) in &mut self.bursts {
            *age += dt;
        }
    ...
    pub fn mesh(&self, visible: impl Fn(Vec3) -> bool) -> MeshData {
        ...
        for &(origin, target, age) in &self.bursts {
            ...
            let axis = if direction.y.abs() > 0.95 {
                Vec3::X
            } else {
                Vec3::Y
            };
            ...
            for i in 0..40 {
                ...
                let p = if i < 20 {
                    ...
                    target
                        + radial * (0.12 + age * 1.3)
                        + direction * ((i % 5) as f32 - 2.) * age * 0.3
                };
                if !visible(p) {
                    continue;
                }
                ...
                push_cuboid(
                    ...
                    p + size,
                    color,
                    atlas::uv_rect(TILE_WHITE),
                );
                for vertex in &mut mesh.vertices[start..] {
                    vertex.emission = fade;
                }
            }
        }
        mesh
