...
pub fn parts(entry: Option<Entry>) -> Vec<(Vec3, Vec3, [f32; 3])> {
    ...
    match entry {
        ...
        Some(Entry::Gear(g)) => {
            ...
            match g {
                Gear::Torch => {
                    add([-0.10, 0.60, -0.10], [0.10, 0.82, 0.10], [1., 0.48, 0.08]);
                    add([-0.05, 0.82, -0.05], [0.05, 0.98, 0.05], [1., 0.85, 0.20]);
                }
                ...
                Gear::Sickle => {
                    add([-0.25, 0.70, -0.04], [0.05, 0.80, 0.04], metal);
                    add([-0.28, 0.45, -0.04], [-0.20, 0.8, 0.04], metal);
                }
                Gear::Spear => {
                    add([-0.04, 0.60, -0.04], [0.04, 1.05, 0.04], wood);
                    add([-0.08, 0.90, -0.03], [0.08, 1.2, 0.03], metal);
                }
                Gear::Dagger => {
                    add([-0.13, 0.32, -0.05], [0.13, 0.38, 0.05], metal);
                    add([-0.055, 0.38, -0.035], [0.055, 0.72, 0.035], metal);
                }
                ...
                Gear::SurveyLantern => {
                    ...
                    add([-0.23, 0.72, -0.18], [0.23, 0.80, 0.18], metal);
                }
                _ => add(
                    [-0.13, 0.65, -0.10],
                    [0.13, 0.9, 0.10],
