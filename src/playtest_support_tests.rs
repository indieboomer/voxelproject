...
fn reproduces_unsupported_decoration_after_legal_support_mining() {
    ...
    let grass_index = crate::voxel::COLLECTIBLE_BLOCKS.iter()
        .position(|b| *b == BlockType::ShortGrass).unwrap();
    actor.player.crafting.resources[grass_index] = 1;
    let snapshot = crate::save::playtest_snapshot(&world, &actor.player,
        &crate::camera::Camera::new(actor.player.position, 1.0), 0.25, vec![],
        &crate::save::CraftingSave::default()).unwrap();
    let mut session = Session::create(actor.player.position, &world, &snapshot,
        &registry, Scenario::MineBlock).unwrap();
    ...
    let mut context = Context { world: &mut world, creatures: &mut creatures,
        loot: &mut loot, registry: &registry, players: &[] };
    let mut rows = Vec::new();
    assert_eq!(recorded_action(&mut actor, &mut context,
        Action::Equip { item: Some(equipment::Entry::Resource(BlockType::ShortGrass)) },
        &mut rows).status, Status::Completed);
    let direction = (Vec3::new(10.5, 61.99, 8.5)
        - (actor.player.position + Vec3::Y * 1.62)).normalize().to_array();
    recorded_action(&mut actor, &mut context, Action::Look { direction }, &mut rows);
    let placed = recorded_action(&mut actor, &mut context,
        Action::Place { target: support, destination: Some(decoration) }, &mut rows);
    ...
    recorded_action(&mut actor, &mut context,
        Action::Equip { item: Some(equipment::Entry::Gear(equipment::Gear::Pickaxe)) }, &mut rows);
    let direction = (center(support) - (actor.player.position + Vec3::Y * 1.62))
        .normalize().to_array();
    recorded_action(&mut actor, &mut context, Action::Look { direction }, &mut rows);
    for _ in 0..8 {
        for _ in 0..4 { recorded_action(&mut actor, &mut context, Action::Wait, &mut rows); }
        let outcome = recorded_action(&mut actor, &mut context, Action::Mine { target: support }, &mut rows);
        assert_ne!(outcome.status, Status::Rejected, "{}", outcome.reason);
        if context.world.get_block(10, 61, 8) == BlockType::Air { break; }
    }
    assert_eq!(context.world.get_block(10, 61, 8), BlockType::Air);
    let block = context.world.get_block(decoration.0, decoration.1, decoration.2);
    let unsupported = block.def().only_on_top
        && !context.world.get_block(decoration.0, decoration.1 - 1, decoration.2).is_solid();
    assert!(unsupported, "Candidate was not reproduced; revisit the issue report");
    let log = rows.iter().map(Value::to_string).collect::<Vec<_>>().join("\n") + "\n";
    ...
    assert!(reproduction["findings_reproduced"].as_array().unwrap().iter().any(|finding|
        finding["invariant"] == "decoration_support" && finding["reproduced"] == true
            && finding["tick"] == actor.tick), "{reproduction}");
    ...
    std::fs::write(session.directory.join("support-issue.json"),
        serde_json::to_vec_pretty(&issue).unwrap()).unwrap();
    session.script.finished = Some("Controlled support issue discovery finished".into());
    eprintln!("Reproduced support issue artifacts: {}", session.directory.display());
}
