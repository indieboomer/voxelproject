...
mod tests {
    ...
    fn spells_assign_with_number_keys_only_when_available() {
        ...
        let module=crate::scripting::Module::load("Heal".into(),"Heal target".into(),include_str!("../modules/target_heal.lua").into()).unwrap();
        let id=book.remember(&module,"Host").unwrap();
        let registry=crate::crafting::Registry::load().unwrap();
        for available in [true,false] {
            let ctx=egui::Context::default();
            let mut account=Account::default();
            if available {book.sync_hotbar(&mut account);}
            let mut inventory=Inventory {selected:Some(Entry::Spell(id)),..Default::default()};
            ...
            let _=ctx.run(egui::RawInput {
                ...
                events:vec![egui::Event::Key {key:egui::Key::Num9,physical_key:None,pressed:true,repeat:false,modifiers:Default::default()}],
                ..Default::default()
            },|ctx|inventory.show(ctx,&account,100.,&registry,&mut requests,&book));
            assert_eq!(requests.assign_entry,available.then_some(Some(Entry::Spell(id))));
        }
    }
    #[test]
    fn numbered_assignment_uses_selection_and_rejects_depleted_items() {
        for (key,slot) in [(egui::Key::Num1,0),(egui::Key::Num9,8)] {
            ...
            let _ = ctx.run(egui::RawInput {
                ...
                ..Default::default()
            }, |ctx|inventory.show(ctx,&account,100.,&registry,&mut requests,&crate::spellbook::Spellbook::default()));
            ...
            let _ = ctx.run(Default::default(), |ctx|inventory.show(ctx,&account,100.,&registry,&mut requests,&crate::spellbook::Spellbook::default()));
...
impl Inventory {
    ...
    fn actions(&self, ui: &mut egui::Ui, account: &Account, health:f32, registry: &crate::crafting::Registry, requests: &mut UiRequests) {
        ...
        match self.selected {
            Some(Entry::Spell(_)) => {
                ...
                if ui.button("Manage spells in Spellbook [K]").clicked() {requests.open_spellbook=true;}
            }
            Some(Entry::Gear(g)) => {
    ...
    pub fn show(&mut self, ctx: &egui::Context, account: &Account, health:f32, registry: &crate::crafting::Registry, requests: &mut UiRequests, book: &crate::spellbook::Spellbook) {
