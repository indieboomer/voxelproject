pub fn clean(value:&str)->String {
    value.chars().filter(|c|c.is_alphanumeric() || matches!(c,' '|'\''|'-')).take(crate::net::MAX_NICKNAME_LEN).collect::<String>().trim().to_string()
}
pub fn fallback()->String {
    let seed=std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().subsec_nanos() as usize;
    let first=["Sir","Dame","Baron","Wizard","Captain","Count"];
    let last=["Wobbleboots","Turnipbeard","Mossbritches","Noodlewand","Picklehelm","Snoreforge"];
...
pub fn request(url:&str)->Result<String,String> {
    ...
    let raw=json["choices"][0]["message"]["content"].as_str().ok_or("No name returned")?;
    if raw.lines().count()!=1 || raw.chars().count()>48 {return Err("Invalid name response".into());}
    let name=clean(raw);if name.is_empty(){Err("Empty name response".into())}else{Ok(name)}
}
#[cfg(test)]
mod tests {
    #[test]
    fn names_are_bounded_and_have_no_control_characters() {
        ...
        for _ in 0..20 {let n=super::fallback();assert!(!n.is_empty());assert_eq!(n,super::clean(&n));}
    }
}
