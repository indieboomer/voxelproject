...
impl BlockFilter {
    ...
    pub fn matches(&self, block: BlockType) -> bool {
        match self {
            Self::Exact(b) => *b == block,
            Self::Any => true,
            Self::Wood => block.is_wood(),
            Self::Ore => block.id().ends_with("_ore") || block == BlockType::Coal,
            Self::Leaves => block.id().ends_with("_leaves"),
            Self::Plant => {
                (block.def().cross || block.def().only_on_top) && block != BlockType::Campfire
            }
            Self::Solid => block.is_solid(),
            Self::Liquid => block == BlockType::Water,
        }
    }
}
...
fn fill(
    ...
    let Some(filter) = BlockFilter::parse(filter.as_deref().unwrap_or("any")) else {
        return Ok(None);
    };
    let lo = std::array::from_fn::<_, 3, _>(|i| a[i].min(b[i]));
    let hi = std::array::from_fn::<_, 3, _>(|i| a[i].max(b[i]));
    let volume = (0..3).fold(1u64, |n, i| {
        n.saturating_mul((hi[i] as i64 - lo[i] as i64 + 1) as u64)
    });
    if volume > 4096
        || lo[1] < 0
        || hi[1] >= CHUNK_Y
        ...
        || lo
            .iter()
            .chain(hi.iter())
            .any(|&v| v.unsigned_abs() > MAX_API_COORDINATE as u32)
    {
        return Ok(None);
    }
        ...
        for y in lo[1]..=hi[1] {
            ...
            for z in lo[2]..=hi[2] {
                if let Some((c, r)) = sphere {
                    if ((x - c[0]).pow(2) + (y - c[1]).pow(2) + (z - c[2]).pow(2)) as f32 > r * r {
                        continue;
                    }
                }
                if !loaded(tx, x, y, z) {
                ...
                if old == block || !filter.matches(old) {
                    continue;
                }
                if old == BlockType::Bedrock || old == BlockType::AutomationDevice {
                    return Ok(None);
                }
                edits.push((x, y, z, block));
                if edits.len() > budget.get() as usize {
                    return Ok(None);
                }
            }
        }
...
pub(super) fn populate<'lua, 'scope>(
    ...
    api.set(
        ...
        scope.create_function(move |lua, id: u32| {
            let Some(p) = tx.players().into_iter().find(|p| p.id == id) else {
                return Ok(None);
            };
            let t = lua.create_table()?;
            set_player_fields(&t, &p)?;
            Ok(Some(t))
        })?,
    )?;
    api.set(
        ...
        scope.create_function(move |lua, id: u32| {
            ...
            let Some(&(_, kind, pos, health, max_health)) =
                creatures.snapshot.iter().find(|c| c.0 == id)
            else {
                return Ok(None);
            };
            let t = lua.create_table()?;
            t.set("id", id)?;
            t.set("kind", creature_kind_name(kind))?;
            t.set("x", pos[0])?;
            t.set("y", pos[1])?;
            t.set("z", pos[2])?;
            t.set("health", health)?;
            t.set("max_health", max_health)?;
            let status = creatures
                .magic_statuses
                .get(&id)
                .copied()
                .unwrap_or_default();
            t.set("slow_seconds", status.slow)?;
            t.set("stun_seconds", status.stun)?;
    ...
    api.set(
        "apply_creature_status",
        scope.create_function(move |lua, (id, status, seconds): (u32, String, f32)| {
    ...
    api.set(
        "get_block_info",
        scope.create_function(move |lua, kind: String| {
            scan_charge(lua, tx, COLLECTIBLE_BLOCKS.len() as u32);
            let Some(b) = BlockType::from_name(&kind) else {
                return Ok(None);
            };
            let d = b.def();
            let t = lua.create_table()?;
            t.set("kind", b.id())?;
            t.set("name", d.display_name)?;
            t.set("resource_type", d.resource_type)?;
            t.set("solid", b.is_solid())?;
            t.set("opaque", b.is_opaque())?;
            t.set("collectible", COLLECTIBLE_BLOCKS.contains(&b))?;
            t.set("hardness", d.hardness)?;
            t.set("emission", d.emission)?;
            t.set("ore", BlockFilter::Ore.matches(b))?;
            Ok(Some(t))
        })?,
    )?;
    api.set(
        "block_matches",
        scope.create_function(move |_, (kind, filter): (String, String)| {
            Ok(BlockType::from_name(&kind)
                .zip(BlockFilter::parse(&filter))
                .is_some_and(|(b, f)| f.matches(b)))
        })?,
    )?;
    api.set(
        "is_block_loaded",
        scope.create_function(move |_, (x, y, z): (i32, i32, i32)| Ok(loaded(tx, x, y, z)))?,
    )?;
    api.set(
        "surface_height",
        scope.create_function(move |lua, (x, z): (i32, i32)| {
            if !loaded(tx, x, 0, z) {
                return Ok(None);
            }
            ...
            Ok((0..CHUNK_Y)
                .rev()
                .find(|&y| tx.get_block(x, y, z).is_solid()))
        })?,
    )?;
    api.set(
        "is_exposed_to_sky",
        scope.create_function(move |lua, (x, y, z): (i32, i32, i32)| {
            ...
            Ok(Some(
                (y..CHUNK_Y).all(|y| !tx.get_block(x, y, z).is_solid()),
            ))
        })?,
    )?;
    api.set(
