...
pub fn caster_account(source: &str) -> Option<String> {
    source
        .lines()
        .find_map(|line| line.strip_prefix(GUEST_CASTER_PREFIX))
...
impl Inbox {
    pub fn submit(
        ...
        if !allowed {
            return Err("Guest prompting is disabled by the host.".into());
        }
        validate_sizes(&prompt, &source)?;
        if self.pending.is_some() {
            return Err("The host is checking another proposal. Try again shortly.".into());
        }
        if self
            .last_submission
            .get(&peer)
        ...
        std::thread::spawn(move || {
            let result = crate::llm::intent::validate_candidate(
                &source,
                crate::llm::classify_prompt(&prompt),
            )
            .map(|()| Proposal {
                prompt,
                source: annotate(&source, &nickname, &account),
                nickname,
            });
...
mod tests {
    ...
    fn proposals_require_permission_and_fit_the_transport_budget() {
        ...
        let packet = crate::net::Packet::Reliable {
            id: 1,
            msg: crate::net::ReliableMsg::RuleProposal {
                prompt: "p".repeat(MAX_PROMPT_BYTES),
        ...
        assert!(matches!(
            crate::net::decode(&bytes),
            Some(crate::net::Packet::Reliable {
                msg: crate::net::ReliableMsg::RuleProposal { .. },
                ..
            })
        ));
    ...
    fn host_worker_rejects_runaway_code_and_keeps_valid_proposals_disabled() {
        ...
        for (source, valid) in [
            ("while true do end", false),
            (
                "function on_cast(api, event) api.set_time_of_day(0.25) end",
                true,
            ),
        ] {
            let mut inbox = Inbox::default();
            ...
            let result = loop {
                if let Some((sender, result)) = inbox.poll() {
