...
pub struct Drop {
    pub pos: [f32; 3],
    pub contents: Vec<(BlockType, u32)>,
    pub age: f32,
    #[serde(default)]
    pub cargo: crate::automation::Inventory,
    #[serde(default)]
    pub machine: bool,
    #[serde(default)]
    pub launch: Option<[f32; 3]>,
}
impl Drop {
    pub fn valid_saved(&self) -> bool {
        if self.machine {
            return self.valid_machine();
        }
        self.pos
            .iter()
            ...
            && self
                .contents
                .iter()
                .all(|(b, n)| crate::voxel::COLLECTIBLE_BLOCKS.contains(b) && *n > 0)
    }
    pub fn display_position(&self) -> Vec3 {
        ...
        if let Some(start) = self.launch {
            ...
            end + Vec3::Y * (0.08 * (self.age * 3.0).sin() + 0.6 * (1.0 - self.age / 0.6).max(0.0))
        }
    }
    pub fn valid_machine(&self) -> bool {
        self.machine
            ...
            && self
                .launch
                .is_some_and(|p| p.iter().all(|v| v.is_finite() && v.abs() < 1_000_000.0))
            && self.age.is_finite()
            && self.age >= 0.0
            && self.contents.len() + self.cargo.len() > 0
            && self.contents.len() <= 64
            && self.cargo.len() <= 64
            && self
                .contents
                .iter()
                .all(|(b, n)| crate::voxel::COLLECTIBLE_BLOCKS.contains(b) && *n > 0 && *n <= 256)
            && self.cargo.iter().all(|(id, n)| {
                crate::automation::valid_item(id)
                    && !id.starts_with("creature:")
                    && *n > 0
                    && *n <= 256
            })
    }
...
fn landing_position(world: &World, pos: Vec3, angle: f32) -> Option<[f32; 3]> {
        ...
        for dz in -radius..=radius {
            for dx in -radius..=radius {
                if dx.abs().max(dz.abs()) == radius {
                    offsets.push(Vec3::new(dx as f32, 0., dz as f32));
                }
            }
        }
    }
    for offset in offsets {
        ...
        let top = (pos.y.ceil() as i32)
            .saturating_add(3)
            .clamp(1, crate::voxel::chunk::CHUNK_Y - 1);
        for y in (0..=top).rev() {
...
pub fn rewards(kind: CreatureKind) -> Vec<(BlockType, u32)> {
    ...
    match kind {
        C::Sheep => vec![(PlantFiber, 2), (Meat, 2)],
        C::Cow => vec![(PlantFiber, 2), (Meat, 4)],
        C::Chicken => vec![(PlantFiber, 1), (Meat, 1)],
        C::Wolf => vec![(PlantFiber, 1), (Resin, 1)],
        C::Stinger => vec![(Resin, 1)],
        C::Goblin => vec![(IronOre, 1), (Cloth, 1)],
        C::StoneGolem => vec![(Stone, 3), (IronOre, 1)],
        C::Sunscorch => vec![(Sulfur, 2), (Ash, 1)],
        C::Zombie => vec![(Ash, 1), (Cloth, 1)],
        C::Skeleton | C::SkeletonSorcerer => vec![(Ash, 2)],
        C::Fish => vec![(PlantFiber, 1), (Meat, 1)],
        C::DragonGreen => vec![(Resin, 3), (CrystalDust, 2)],
        C::DragonRed => vec![(Sulfur, 3), (CrystalDust, 2)],
    }
}
...
mod tests {
    ...
    fn planted_scatter_cells_use_nearby_clear_ground_and_allow_pickup() {
        ...
        fx.spawn(&world, CreatureKind::Sheep, Vec3::new(8., 25., 8.));
        assert_eq!(fx.drops.len(), 1);
        assert_eq!(fx.drops[0].pos, [11.5, 25.3, 8.5]);
        ...
        assert_eq!(
            fx.collect(&world, Vec3::new(11.5, 25., 8.5), &mut account),
            rewards(CreatureKind::Sheep)
        );
    }
    #[test]
    fn fully_occupied_ground_is_not_used_or_cleared() {
        ...
        for x in 0..16 {
            for z in 0..16 {
                chunk.set_local(x, 24, z, BlockType::Stone);
                chunk.set_local(x, 25, z, BlockType::WildHerbs);
            }
        }
        world.chunks.insert((0, 0), chunk);
    ...
    fn bundle_collection_is_atomic_and_only_rewards_one_player() {
        ...
        let mut account = crate::crafting::Account::default();
        let iron = crate::voxel::COLLECTIBLE_BLOCKS
            .iter()
            .position(|b| *b == BlockType::IronOre)
            .unwrap();
        ...
        assert_eq!(
            ...
            rewards(CreatureKind::StoneGolem)
        );
    ...
    fn drops_find_sloping_ground_and_fall_back_at_loaded_edges() {
        ...
        for y in 0..=27 {
            chunk.set_local(9, y, 8, BlockType::Stone);
        }
        chunk.set_local(8, 24, 8, BlockType::Stone);
        world.chunks.insert((0, 0), chunk);
        assert_eq!(
            landing_position(&world, Vec3::new(8.0, 25.0, 8.0), 0.0),
            Some([9.5, 28.3, 8.5])
        );
        ...
        assert_eq!(
            landing_position(&world, Vec3::new(16.0, 25.0, 0.0), std::f32::consts::PI),
            Some([16.5, 25.3, 0.5])
        );
    }
    #[test]
    ...
    fn every_drop_has_an_inventory_stack_texture_and_elemental_composition() {
        ...
        for kind in 0..=12 {
            for (block, amount) in rewards(CreatureKind::from_u8(kind)) {
                ...
                let uv = atlas::uv_rect(atlas::tile_for(block, 0));
    ...
    fn scattered_loot_awards_once_preserves_full_inventory_and_expires() {
        ...
        fx.spawn(&world, CreatureKind::StoneGolem, Vec3::new(8.0, 25.0, 8.0));
        assert_eq!(fx.drops.len(), 1);
        assert_eq!(fx.drops[0].contents.len(), 2);
        fx.update(PICKUP_DELAY, true);
        let mut account = crate::crafting::Account::default();
        for drop in fx.drops.clone() {
            fx.collect(&world, Vec3::from_array(drop.pos), &mut account);
        }
        assert!(fx.drops.is_empty());
        let before = account.clone();
        fx.collect(&world, Vec3::new(8.0, 25.0, 8.0), &mut account);
        assert_eq!(before, account);
        fx.spawn(&world, CreatureKind::Sheep, Vec3::new(8.0, 25.0, 8.0));
        fx.update(PICKUP_DELAY, true);
        account.resources.fill(u32::MAX);
        let pos = Vec3::from_array(fx.drops[0].pos);
        fx.collect(&world, pos, &mut account);
        assert!(!fx.drops.is_empty());
        for _ in 0..100 {
            fx.spawn(&world, CreatureKind::Sheep, pos);
            fx.puff(pos);
        }
        assert!(fx.drops.len() <= 48 && fx.puffs.len() <= 16);
        assert!(!fx.mesh(|_| true).vertices.is_empty());
        fx.update(121.0, true);
        assert!(fx.drops.is_empty() && fx.puffs.is_empty());
    }
}
impl Effects {
    pub fn puff(&mut self, pos: Vec3) {
        if self.puffs.len() >= 16 {
            self.puffs.remove(0);
        }
        self.puffs.push((pos, 0.0));
    }
    pub fn spawn(&mut self, world: &World, kind: CreatureKind, pos: Vec3) {
        if self.drops.len() >= 48 {
            return;
        }
        let angle = pos.x * 1.7 + pos.z * 0.8;
        let Some(pos) = landing_position(world, pos, angle) else {
            return;
        };
        self.drops.push(Drop {
            pos,
            contents: rewards(kind),
            age: 0.0,
    ...
    pub fn restore_machine(drops: Vec<Drop>) -> Self {
        Self {
            drops,
            puffs: Vec::new(),
        }
    ...
    pub fn eject(
        ...
        if self.drops.len() >= 48
            || amount == 0
            || amount > 256
            || !crate::automation::valid_item(item)
            || item.starts_with("creature:")
        {
            return false;
        }
        ...
        for attempt in 0..8 {
            ...
            let Some(pos) = landing_position(world, start, angle) else {
                continue;
            };
            let end = Vec3::from_array(pos);
            if glam::Vec2::new(end.x - start.x, end.z - start.z).length() < 0.8
                || end.distance(start) > 5.0
            {
                continue;
            }
            let mut drop = Drop {
                ...
                cargo: Default::default(),
                machine: true,
                launch: Some(start.to_array()),
            };
            if let Some(block) = item
                .strip_prefix("resource:")
                .and_then(BlockType::from_name)
            {
                drop.contents.push((block, amount));
            } else {
                drop.cargo.insert(item.into(), amount);
            }
            let clear = (0..=30).all(|i| {
                drop.age = i as f32 / 30.0 * 0.85;
                let p = drop.display_position();
                [
                    Vec3::ZERO,
                    Vec3::X * 0.18,
                    -Vec3::X * 0.18,
    ...
    pub fn update(&mut self, dt: f32, host: bool) {
        for (_, age) in &mut self.puffs {
            *age += dt;
        }
        self.puffs.retain(|(_, age)| *age < 0.6);
        for d in &mut self.drops {
            d.age = (d.age + dt.max(0.0)).min(3600.0);
        }
        if host {
            self.drops.retain(|d| d.machine || d.age < 120.0);
        }
    }
    pub fn collect(
        &mut self,
        ...
        self.drops.retain(|d| {
            let pos = Vec3::from_array(d.pos);
            let eye = feet + Vec3::Y * 1.4;
            if d.age < PICKUP_DELAY
                || pos.distance(feet) > 1.5
                || crate::raycast::raycast(world, eye, pos - eye, (pos - eye).length() - 0.25)
                    .is_some()
            {
                return true;
            }
            ...
            for &(block, amount) in &d.contents {
                let Some(i) = crate::voxel::COLLECTIBLE_BLOCKS
                    .iter()
                    .position(|b| *b == block)
                else {
                    return true;
                };
                let Some(count) = next.resources[i].checked_add(amount) else {
                    return true;
                };
                next.resources[i] = count;
            }
            for (item, n) in &d.cargo {
                if crate::automation::account_add(&mut next, item, *n).is_err() {
                    return true;
                }
            }
            let Some(revision) = next.revision.checked_add(1) else {
                return true;
            };
    ...
    pub fn mesh(&self, visible: impl Fn(Vec3) -> bool) -> MeshData {
        ...
        for d in &self.drops {
            ...
            mesh.vertices.extend(bag.vertices.iter().map(|v| {
                let mut v = *v;
                v.position = (Vec3::from_array(v.position) + p).to_array();
                v
            }));
            ...
            for i in 0..3 {
                let phase =
                    (d.age * 0.55 + i as f32 / 3.0 + p.x * 0.13 + p.z * 0.19).rem_euclid(1.0);
                let angle = i as f32 * 2.4 + phase * 0.7;
                let center =
                    p + Vec3::new(angle.cos() * 0.18, 0.18 + phase * 0.65, angle.sin() * 0.18);
        ...
        for (origin, age) in &self.puffs {
            ...
            for i in 0..12 {
                ...
                let p = *origin
                    + Vec3::Y * 0.9
                    + Vec3::new(a.cos(), (i % 3) as f32 * 0.5, a.sin()) * (0.2 + t * 1.8);
                ...
                push_cuboid(
                    ...
                    p + Vec3::splat(size),
                    [0.85; 3],
                    atlas::uv_rect(crate::voxel::atlas_tiles::TILE_WHITE),
                );
                for v in &mut mesh.vertices[start..] {
                    v.emission = 0.7;
                }
