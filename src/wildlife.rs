...
impl Creatures {
    ...
    pub(super) fn has_population_room(&self) -> bool {
        self.ecs.len() < 128
    }
    ...
    pub fn populate_wildlife(&mut self, world: &World, players: &[(PlayerId, Vec3)], dt: f32) {
        ...
        self.wildlife.retain(|id, cell| {
            let Some(c) = snapshot.iter().find(|c| c.0 == *id) else {
                return false;
            };
            let kind = CreatureKind::from_u8(c.1);
            let custom = self.behaviors.get(id).is_some_and(|b| {
                b.mode != BehaviorMode::Auto
                    || b.target.is_some()
                    || b.aggressive != kind.is_hostile()
            });
            if custom {
                return false;
            } // Modified wildlife becomes persistent.
            if players
                .iter()
                .all(|(_, p)| distance(*p, Vec3::from_array(c.2)) > 128.0 * 128.0)
            {
                remove.push((*id, *cell));
                return false;
            }
        ...
        for (id, cell) in remove {
            ...
            if let Some(cell) = cell {
                self.fish_regions.remove(&cell);
            }
        }
        self.population_sequence = self.population_sequence.wrapping_add(1);
        let mut rng = SimpleRng::new(
            world.seed as u64 ^ self.population_sequence.wrapping_mul(0x9E3779B185EBCA87),
        );
        // Rotate player priority to avoid starving distant guests at the global cap.
        for offset in 0..players.len() {
            let (_, center) = players
                [(offset + self.population_sequence as usize % players.len()) % players.len()];
            let mut nearby = self
                .snapshot_with_ids()
                .iter()
                .filter(|c| {
                    !CreatureKind::from_u8(c.1).is_dragon()
                        && CreatureKind::from_u8(c.1) != CreatureKind::Fish
                        && distance(center, Vec3::from_array(c.2)) < 88.0 * 88.0
                })
                .count();
            for _ in 0..2 {
                if nearby >= 20 || !self.has_population_room() {
                    break;
                }
                ...
                for _ in 0..12 {
                    ...
                    if !world.chunks.contains_key(&world_to_chunk(x, z)) {
                        continue;
                    }
                    let Some(y) = (1..CHUNK_Y - 4).rev().find(|&y| {
                        ...
                        ) && (0..4).all(|dy| world.get_block(x, y + dy, z) == BlockType::Air)
                    }) else {
                        continue;
                    };
                    ...
                    if players.iter().any(|(_, p)| distance(*p, pos) < 48.0 * 48.0)
                        || self
                            ...
                            .query::<&Pos>()
                            .iter()
                            .any(|(_, p)| distance(p.0, pos) < 8.0 * 8.0)
                    {
                        continue;
                    }
                    let id = self.spawn_one(
                        kind,
                        pos,
                        self.population_sequence ^ x as u64 ^ (z as u64).rotate_left(32),
                    );
...
mod tests {
    ...
    fn exploration_recycles_only_natural_wildlife_and_stays_bounded() {
        ...
        for region in 0..12 {
            ...
            for _ in 0..15 {
                creatures.populate_wildlife(&world, &[(7, player)], 2.0);
            }
            ...
            for c in snapshot
                .iter()
                .filter(|c| creatures.wildlife.contains_key(&c.0))
            {
                let p = Vec3::from_array(c.2);
                assert!(p.distance(player) >= 48.0 && p.distance(player) <= 73.0);
                assert_eq!(
                    world.get_block(p.x.floor() as i32, 24, p.z.floor() as i32),
                    BlockType::Stone
                );
            }
    ...
    fn discovery_respects_global_cap_and_unloaded_ground() {
        ...
        for i in 0..128 {
            creatures.spawn_one(CreatureKind::Sheep, Vec3::new(-1000.0, 25.0, i as f32), i);
        }
