...
impl App {
    ...
    pub(super) fn perform_eat_harvest(&mut self, from: Option<Peer>, item: String, revision: u64) {
        ...
        if let Ok(delta) = result { self.apply_player_effect(PlayerEffect::Health { player_id: id, delta }); if matches!(item.as_str(), "harvest:glowcap" | "harvest:cooked_glowcap" | "harvest:fired_glowcap" | crate::food::CAMPFIRE_POISONOUS_DISH) { self.apply_player_effect(PlayerEffect::Poisoned { player_id: id, poisoned: true }); } else if item == crate::food::CAMPFIRE_HERBAL_DISH { self.apply_player_effect(PlayerEffect::Poisoned { player_id: id, poisoned: false }); } let satiety = crate::food::harvest_satiety(&item); if from.is_none() { self.player.restore_satiety(satiety); } else if let NetRole::Host(host) = &mut self.net { if let Some(p) = host.remote_players.get_mut(&id) { crate::hunger::eat(&mut p.satiety, satiety); } } }
            ...
            if let Some(peer) = from {
                if let NetRole::Host(host) = &mut self.net {
                    if let Some(player) = host.remote_players.get_mut(&id) {
                        crate::hunger::eat(&mut player.satiety, satiety);
                    }
                }
                let _ = peer;
            } else {
