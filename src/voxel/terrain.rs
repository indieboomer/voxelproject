...
pub fn geological_height(x: i32, z: i32, seed: u32, h: i32, relief: f32, version: u8) -> i32 {
    ...
    let rubble = (fbm(x * 0.29, z * 0.29, seed ^ 0x6E04, 2, 2., 0.5) - 0.5)
        * if version >= 2 { 1.0 } else { 3.0 };
    (h as f32 + inland * (ledge - fissure - basin + rubble) * relief.min(1.5))
        .round()
        .clamp(2., (CHUNK_Y - 4) as f32) as i32
}
...
pub fn meadow_plant(x: i32, z: i32, seed: u32) -> Option<super::BlockType> {
    ...
    for a in cx - 1..=cx + 1 {
        for b in cz - 1..=cz + 1 {
            if column_rand(a, b, seed, 0xF100) > 0.72 {
                continue;
            }
            ...
            if column_rand(x, z, seed, 0xF106) > ((1.15 - d + edge) * 0.55).clamp(0., 0.65) {
                continue;
            }
            let mixed =
                column_rand(a, b, seed, 0xF107) < 0.45 || column_rand(x, z, seed, 0xF108) > 0.88;
            let roll = if mixed {
                column_rand(x, z, seed, 0xF109)
            } else {
                column_rand(a, b, seed, 0xF110)
            };
...
pub(super) fn mountain_surface_with_slope(
    wx: i32,
    wz: i32,
    h: i32,
    seed: u32,
...
fn lake_x(column: i32, row: i32, seed: u32) -> f32 {
    column as f32 * LAKE_SPACING + 140.0 + (column_rand(column, row, seed, 0xA712) - 0.5) * 110.0
}
...
pub fn tributary(wx: i32, wz: i32, seed: u32) -> Option<(i32, i32, bool)> {
    ...
    for row in r - 1..=r + 1 {
        for column in c - 1..=c + 1 {
            if column_rand(column, row, seed, 0xFA110) > 0.55 {
                continue;
            }
            ...
            if z < 14.0 {
                ...
                if x <= radius {
                    return Some((SEA_LEVEL - 3, SEA_LEVEL, false));
                }
                continue;
            }
            ...
            let rim_distance = if z <= 32.0 {
                (x - 2.0).max(0.0).min(pool_distance - 7.0)
            } else {
                pool_distance - 7.0
            };
            if rim_distance < 5.0 && pool_distance < 12.0 || (z <= 32.0 && x < 7.0) {
                return Some((
                    high + 1 - (rim_distance - 2.0).max(0.0) as i32,
                    SEA_LEVEL,
                    false,
                ));
            }
...
pub fn current_with_lakes(
    ...
    if elevated {
        if let Some((bed, level, flow)) = tributary(wx, wz, seed) {
            if flow && level > SEA_LEVEL && bed < level {
                return [0.0, -1.0];
            }
        }
        return [0.0; 2];
    ...
    for r in row - 1..=row + 1 {
        if (z - river_z(x, r, seed)).abs() > 4.0 {
            continue;
        }
        for c in column - 1..=column + 1 {
            let (lx, lz, rx, rz) = lake(c, r, seed);
            let distance = if irregular {
                lake_distance(x, z, c, r, seed)
            } else {
                ((x - lx) / rx).powi(2) + ((z - lz) / rz).powi(2)
            };
            if distance < 1.25 {
                return [0.0; 2];
            }
...
fn lake_distance(x: f32, z: f32, column: i32, row: i32, seed: u32) -> f32 {
    ...
    if (x - lx).abs() > rx * 3.5 || (z - lz).abs() > rx * 3.5 {
        return 4.;
    }
    ...
    let u = (dx * c + dz * s) / (rx * stretch);
    let v = (-dx * s + dz * c) / (rz / stretch);
    let theta = v.atan2(u);
    let phase = column_rand(column, row, seed, 0x1A03) * std::f32::consts::TAU;
    let radius = 1.
        + 0.26 * (theta * 2. + phase).sin()
        + 0.16 * (theta * 3. - phase * 1.7).cos()
        + 0.07 * (theta * 5. + phase * 0.8).sin();
...
pub fn height_with_lakes(wx: i32, wz: i32, seed: u32, irregular: bool) -> i32 {
    ...
    for r in row - 1..=row + 1 {
        ...
        for c in column - 1..=column + 1 {
            ...
            if irregular && ((x - lx).abs() > rx * 3.5 || (z - lz).abs() > rx * 3.5) {
                continue;
            }
            let distance = if irregular {
                lake_distance(x, z, c, r, seed)
...
pub fn tributary_preview(seed: u32) -> (i32, i32) {
    let c = (-4..4)
        .find(|&c| column_rand(c, 0, seed, 0xFA110) <= 0.55)
        .unwrap();
...
mod tests {
    ...
    fn irregular_lakes_keep_connected_interiors_and_original_uplands() {
        ...
        for seed in [7, 42, 2026] {
            for c in [-1, 0, 1] {
                ...
                assert_eq!(
                    current_with_lakes(lx.round() as i32, lz.round() as i32, seed, false, true),
                    [0.; 2]
                );
                ...
                for ray in 0..48 {
                    ...
                    for r in 0..160 {
                        ...
                        if lake_distance(x, z, c, 0, seed) >= 0.75 {
                            break;
                        }
                        assert!(
                            height_with_lakes(x.round() as i32, z.round() as i32, seed, true)
                                < SEA_LEVEL,
                            "dry lake interior seed={seed} at {x},{z}"
                        );
                        radius = r;
                    }
                    radii.push(radius);
                ...
                assert!(
                    radii.iter().max().unwrap() * 10 > radii.iter().min().unwrap() * 17,
                    "lake too round: {radii:?}"
                );
            }
            let config = crate::worldgen::WorldGeneration::default();
            let old = crate::worldgen::WorldGeneration {
                landscape_version: 0,
                ..Default::default()
            };
            for x in (-256..256).step_by(8) {
                for z in (-256..256).step_by(8) {
                    if config.height(x, z, seed) != old.height(x, z, seed) {
                        changed += 1;
                        let column = (x as f32 / LAKE_SPACING).floor() as i32;
                        let row = (z as f32 / RIVER_SPACING).floor() as i32;
                        assert!(
                            (column - 1..=column + 1).any(|c| (row - 1..=row + 1).any(|r| {
                                let (lx, lz, rx, rz) = lake(c, r, seed);
                                lake_distance(x as f32, z as f32, c, r, seed) < 2.
                                    || ((x as f32 - lx) / rx).powi(2)
                                        + ((z as f32 - lz) / rz).powi(2)
                                        < 2.
                            })),
    ...
    fn lake_shape_preview() {
        ...
        for row in 0..2 {
            for column in 0..3 {
                let seed = [7, 42, 2026][column];
                let (lx, lz, _, _) = lake(0, 0, seed);
                for x in 0..256 {
                    for z in 0..256 {
                        let h = height_with_lakes(
                            lx as i32 + x - 128,
                            lz as i32 + z - 128,
                            seed,
                            row == 1,
                        ...
                        let color = if h < SEA_LEVEL {
                            ...
                            [65, 125 + (h - 18).max(0) as u8 * 2, 65]
                        };
                        image.put_pixel(
                            column as u32 * 256 + x as u32,
                            row * 256 + z as u32,
                            image::Rgb(color),
                        );
                    }
    ...
    fn wider_formations_reduce_short_distance_height_changes() {
        for seed in [7, 42, 2026] {
            let variation = |version| {
                ...
                for z in [-128, 0, 128] {
                    for x in -512..512 {
                        total += (geological_height(x + 1, z, seed, 32, 1., version)
                            - geological_height(x, z, seed, 32, 1., version))
                        .abs();
                    }
                }
                total
            };
            let compact = variation(1);
            let broad = variation(2);
            assert!(
                broad * 100 < compact * 65,
                "seed={seed}, compact={compact}, broad={broad}"
            );
        }
    }
    #[test]
    fn geological_detail_has_cliffs_cuts_and_supported_overhangs() {
        ...
        for x in (-192..192).step_by(3) {
            for z in (-192..192).step_by(3) {
                ...
                let old = height(x, z, 42);
                changed += usize::from(h != old);
                depressions += usize::from(h <= old - 3);
                let slope = [
                    config.height(x - 1, z, 42),
                    config.height(x + 1, z, 42),
                    config.height(x, z - 1, 42),
                    config.height(x, z + 1, 42),
                ]
                .into_iter()
                .map(|n| (n - h).abs())
                ...
                if undercut(x, z, 42, h, slope, config.landscape_version) {
                    candidates.push((x, z, h));
                }
            }
        }
        assert!(
            changed > 1000 && cliffs > 20 && depressions > 50,
            "changed={changed} cliffs={cliffs} depressions={depressions}"
        );
        let mut world = World::new(42);
        world.generation = config;
        world.generation.underground = false;
        let mut roofs = 0;
        for &(x, z, h) in candidates.iter().take(40) {
            let (cx, cz) = crate::voxel::chunk::world_to_chunk(x, z);
            world.ensure_chunk_loaded(cx, cz);
            if world.get_block(x, h - 3, z) == BlockType::Air
                && world.get_block(x, h - 1, z).is_solid()
            {
                roofs += 1;
            }
        }
        assert!(
            roofs > 0,
            "No generated overhang roofs among {} candidates",
            candidates.len()
        );
    }
    #[test]
    fn meadows_include_mixed_and_dominant_colonies() {
        ...
        for a in -5..5 {
            for b in -5..5 {
                ...
                for x in px - 6..=px + 6 {
                    for z in pz - 6..=pz + 6 {
                        if let Some(plant) = meadow_plant(x, z, 42) {
                            *counts.entry(plant.id()).or_insert(0) += 1;
                        }
                    }
                }
        ...
        for seed in [7, 42, 2026] {
            for row in [-1, 0, 1] {
                for c in -1..=1 {
                    ...
                    assert_eq!(
                        current(x.round() as i32, z.round() as i32, seed, false),
                        [0.0; 2]
                    );
                }
                let mut moving = 0;
    ...
    fn generated_tributaries_have_contained_pools_and_real_spills_across_chunks() {
        use super::super::{chunk::world_to_chunk, BlockType, World};
        for seed in [7, 42, 2026] {
            let column = (-4..4)
                .find(|&c| column_rand(c, 0, seed, 0xFA110) <= 0.55)
                .unwrap();
            ...
            let falls: Vec<_> = world
                .chunks
                .values()
                .flat_map(|chunk| crate::water::scan_chunk(&world, chunk))
                .collect();
            assert!(falls.iter().any(
                |f| (f.lip.x - x as f32).abs() < 3.0 && (f.lip.z - (z + 14) as f32).abs() < 1.0
            ));
    ...
    fn snowy_summits_have_solid_caps_and_preserve_player_edits() {
        ...
        for x in (-512..512).step_by(8) {
            for z in (-512..512).step_by(8) {
                ...
                .max()
                .unwrap();
                if h >= 40 && mountain_surface_with_slope(x, z, h, 42, slope) == BlockType::Snow {
                    summit = Some((x, z, h));
                    break;
                }
            }
        ...
        assert!(BlockType::Snow.is_solid());
        assert!(BlockType::Snow.hand_pickable());
        assert_eq!(BlockType::from_name("snow"), Some(BlockType::Snow));
        world.set_block(x, h, z, BlockType::Bricks);
        world.unload_chunk(cx, cz);
        world.ensure_chunk_loaded(cx, cz);
    ...
    fn terrain_survey_has_sparse_high_peaks_and_water() {
        ...
        for x in 0..512 {
            for z in 0..512 {
                ...
                if h >= ROCK_LINE {
                    match surface {
                        super::super::BlockType::Snow => snow += 1,
                        super::super::BlockType::Stone => bare_rock += 1,
                        _ => alpine_grass += 1,
                    }
        ...
        assert!(
            snow > 100 && bare_rock > 100,
            "snow={snow}, rock={bare_rock}"
        );
        assert!(
            alpine_grass > 0 && alpine_grass < rocky / 10,
            "grass={alpine_grass}/{rocky}"
        );
