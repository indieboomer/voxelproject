...
pub(super) fn populate<'lua, 'scope>(
    ...
    api.set(
        ...
        scope.create_function(move |lua, (id, radius): (u32, f32)| {
            if blocks.get() == 0 {
                return Ok(None);
            }
            ...
            let radius = radius.clamp(2.0, 8.0);
            ...
            for dx in -r..=r {
                for dz in -r..=r {
                    ...
                    if distance > radius * radius || distance < 4.0 {
                        continue;
                    }
                    for dy in -4i32..=4 {
                        candidates.push((dx * dx + dz * dz + dy * dy, cx + dx, cy + dy, cz + dz));
                    }
                }
            }
            ...
            for (_, x, y, z) in candidates {
                ...
                if players
                    .iter()
                    .any(|p| (p.pos.x - center.x).powi(2) + (p.pos.z - center.z).powi(2) < 4.0)
                {
                    continue;
                }
                if !matches!(
                ...
                if existing != BlockType::Air
                    && !(existing.def().only_on_top && existing != BlockType::Campfire)
                {
                    continue;
                }
                if (1..4).any(|dy| tx.get_block(x, y + dy, z) != BlockType::Air) {
                    continue;
                }
