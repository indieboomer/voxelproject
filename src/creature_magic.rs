...
impl Status {
    pub fn valid(&self) -> bool {
        [self.slow, self.stun]
            .iter()
            .all(|t| t.is_finite() && (0.0..=30.0).contains(t))
    }
    pub fn active(&self) -> bool {
        self.slow > 0. || self.stun > 0.
    }
    ...
    pub fn tick(&mut self, dt: f32) {
        self.slow = (self.slow - dt).max(0.);
        self.stun = (self.stun - dt).max(0.);
    }
}
impl CreatureDraft {
    pub fn apply_status(&mut self, id: u32, name: &str, seconds: f32) -> bool {
        if !seconds.is_finite()
            ...
            || !self.snapshot.iter().any(|c| c.0 == id)
        {
            return false;
        }
        let mut status = self.magic_statuses.get(&id).copied().unwrap_or_default();
        match name {
            "slow" => status.slow = seconds,
            "stun" => status.stun = seconds,
            _ => return false,
        }
        self.magic_statuses.insert(id, status);
        self.commands.push(CreatureCommand::Status(id, status));
        true
    }
    ...
    pub fn push(
        ...
        id: u32,
        delta: Vec3,
        clear_block: impl Fn(i32, i32, i32) -> Option<BlockType>,
    ) -> bool {
        if !delta.is_finite() || delta.length() > 8. || delta.length() < 0.001 {
            return false;
        }
        ...
        for step in 1..=steps {
            ...
            for x in (next.x - radius).floor() as i32..=(next.x + radius - 0.001).floor() as i32 {
                ...
                {
                    for y in
                        (bottom + 0.001).floor() as i32..=(bottom + height - 0.001).floor() as i32
                    {
                        if !clear_block(x, y, z).is_some_and(|b| !b.is_solid()) {
                            clear = false;
                        }
                    }
                }
        ...
        if end.distance_squared(start) < 0.000001 {
            return false;
        }
        entry.2 = end.to_array();
        self.commands.push(CreatureCommand::Move(id, end));
        true
    }
}
...
mod tests {
    ...
    fn scene() -> (World, Creatures, u32) {
        ...
        let id = c.spawn_one(CreatureKind::Sheep, Vec3::new(5., 25., 5.), 1);
        c.set_chase_target(id, Vec3::new(12., 25., 5.));
        (world, c, id)
    }
    #[test]
    fn slow_changes_motion_stun_expires_and_saved_status_retains_duration() {
        let (world, mut normal, id) = scene();
        let (_, mut slowed, _) = scene();
        slowed.magic_statuses.insert(
            id,
            Status {
                slow: 10.,
                stun: 0.,
        ...
        normal.update(&world, 0.1, &[]);
        slowed.update(&world, 0.1, &[]);
        let normal_dx = normal.snapshot_with_ids()[0].2[0] - 5.;
        let slow_dx = slowed.snapshot_with_ids()[0].2[0] - 5.;
        ...
        slowed.update(&world, 0.1, &[]);
        assert_eq!(slowed.snapshot_with_ids()[0].2, original);
        let save = crate::save::CraftingSave {
            creature_statuses: slowed.magic_statuses.clone(),
            ..Default::default()
        };
        ...
        slowed.update(&world, 0.1, &[]);
        assert_eq!(slowed.magic_statuses[&id].stun, 0.);
        slowed.update(&world, 0.1, &[]);
        assert!(slowed.snapshot_with_ids()[0].2[0] > original[0]);
        slowed.update(&world, 30., &[]);
        assert!(slowed.magic_statuses.is_empty());
    }
    #[test]
    fn pushes_reject_unloaded_space_and_nonfinite_or_excessive_displacements() {
        let (_, c, id) = scene();
        let mut draft = CreatureDraft::new(&c);
