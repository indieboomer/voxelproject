...
impl Finding {
    pub fn new(tick: u64, invariant: &str, expected: &str, observed: String) -> Self {
        Self { invariant: invariant.into(), tick,
            event_reference: format!("events.jsonl:outcome.tick={tick}"),
            expected: expected.into(), observed, basis: "transaction invariant".into(),
            classification: "uncertain".into(), severity: "high".into(),
...
fn balances(a: &crafting::Account) -> Vec<i64> {
    a.resources.iter().chain(a.elements.iter()).chain(a.gear.iter())
        .copied().chain(std::iter::once(a.mana)).map(i64::from).collect()
}
fn resource(delta: &mut [i64], block: BlockType, amount: i64) {
    if let Some(i) = COLLECTIBLE_BLOCKS.iter().position(|b| *b == block) { delta[i] += amount; }
}
...
pub fn support(tick: u64, world: &crate::voxel::World, effects: &Effects) -> Vec<Finding> {
    ...
    for (cell, new, old) in &effects.edits {
        if !old.is_solid() || new.is_solid() { continue; }
        let above = (cell.0, cell.1 + 1, cell.2);
        let block = effects.edits.iter().find(|(p, _, _)| *p == above)
            .map_or_else(|| world.get_block(above.0, above.1, above.2), |(_, b, _)| *b);
        if block.def().only_on_top {
            let mut finding = Finding::new(tick, "decoration_support",
                "Blocks marked only_on_top must sit on solid ground",
                format!("Direct edit at {cell:?} leaves {} at {above:?} above {}", block.id(), new.id()));
...
pub fn embedded(world: &crate::voxel::World, feet: glam::Vec3) -> bool {
    if !feet.is_finite() { return false; }
...
pub fn transaction(tick: u64, action: &Action, before: &crafting::Account,
    after: &crafting::Account, rejected: bool, effects: &Effects,
    registry: &crafting::Registry) -> Vec<Finding> {
    let mut findings = Vec::new();
    if rejected {
        if before != after || !effects.edits.is_empty() {
            findings.push(Finding::new(tick, "rejected_action_atomicity",
                "Rejected actions preserve inventory and emit no block edits",
                format!("Inventory changed: {}; edits: {:?}", before != after, effects.edits)));
        }
        return findings;
    }
    let initial = balances(before);
    let actual: Vec<i64> = balances(after).iter().zip(&initial).map(|(a,b)| a-b).collect();
    ...
    let checked = match action {
        ...
        Action::Mine { .. } => {
            for (_, new, old) in &effects.edits {
                ...
                } else { resource(&mut expected, *old, 1); }
            }
            true
        }
        Action::Place { .. } => {
            for (_, block, _) in &effects.edits { resource(&mut expected, *block, -1); }
            true
        }
        Action::Craft { recipe } => {
            match recipe {
                ...
                crafting::Action::Craft(slots) => {
                    ...
                    for slot in &recipe.inputs { expected[element + slot.element as usize] -= slot.amount; }
                    expected[mana] -= i64::from(registry.mana_cost(slots));
                ...
                crafting::Action::Extract { block, amount } => {
                    ...
                    for (i, n) in registry.composition(crafting::ObjectKind::Resource, block.id()).iter().enumerate() {
...
mod tests {
    ...
    fn detects_free_tool_and_rejected_spending_without_trusting_outcome_text() {
        ...
        let action = Action::Craft { recipe: crafting::Action::CraftGear(equipment::Gear::Axe) };
        assert_eq!(transaction(1, &action, &before, &after, false, &Effects::default(), &registry)[0].invariant, "resource_balance");
        assert_eq!(transaction(1, &action, &before, &after, true, &Effects::default(), &registry)[0].invariant, "rejected_action_atomicity");
        assert!(transaction(1, &action, &before, &before, true, &Effects::default(), &registry).is_empty());
    }
}
