...
pub fn icon(ui: &mut egui::Ui, entry: Entry) {
    ...
    if let Entry::Spell(id)=entry {
        ...
        let points=vec![c+egui::vec2(0.,-11.),c+egui::vec2(9.,0.),c+egui::vec2(0.,11.),c+egui::vec2(-9.,0.)];
        ui.painter().add(egui::Shape::convex_polygon(points,egui::Color32::from_rgb(60,30,90),egui::Stroke::new(1.5_f32,color)));
        ui.painter().circle_filled(c,3.,egui::Color32::from_rgb(255,220,140));
        for i in 0..(id%3+1) {ui.painter().line_segment([c+egui::vec2(-3.+i as f32*3.,-7.),c+egui::vec2(-3.+i as f32*3.,-4.)],egui::Stroke::new(1_f32,color));}
        return;
    }
...
pub fn hotbar(
    ...
    hotbar_with_spells(ctx,account,interactive,show_name,disabled,&crate::spellbook::Spellbook::default())
}

pub fn entry_name(entry:Entry,book:&crate::spellbook::Spellbook)->&str {
    if let Entry::Spell(id)=entry {book.get(id).map_or("Unavailable spell",|s|s.name.as_str())}else{entry.name()}
}

pub fn spell_hud(ctx:&egui::Context,text:&str,ready:bool) {
    egui::Area::new(egui::Id::new("spell_cast_hud")).anchor(egui::Align2::CENTER_BOTTOM,[0.,-116.]).show(ctx,|ui|{
        egui::Frame::none().fill(egui::Color32::from_black_alpha(190)).inner_margin(6.).show(ui,|ui|{
            ui.colored_label(if ready {egui::Color32::LIGHT_GREEN}else{egui::Color32::from_rgb(255,200,125)},text);
        });
    });
}

pub fn hotbar_with_spells(
    ctx:&egui::Context, account:&crate::crafting::Account, interactive:bool,
    show_name:bool, disabled:bool, book:&crate::spellbook::Spellbook,
) -> Option<usize> {
        ...
        .show(ctx, |ui| {
            ...
            if show_name || disabled || matches!(entry,Some(Entry::Spell(_))) {
                ...
                ui.painter().text(
                    ...
                    if disabled { "Tools paused — exit machine mode to restore" } else { entry.map_or("Empty hand", |e| entry_name(e,book)) },
            ...
            ui.horizontal(|ui| {
                ...
                for i in 0..9 {
                    ...
                    if let Some(e) = account.hotbar.slots[i].filter(|e|!matches!(e,Entry::Gear(g) if !g.enabled() || *g==Gear::Torch)) {
                        ...
                        ui.painter().text(
                            ...
                            if matches!(e,Entry::Spell(_)) {if count>0 {"LMB".into()} else {"!".into()}} else {format!("{count}")},
                            egui::FontId::proportional(14.0),
                            if count > 0 {
                                ivory
                            } else {
